...
import {
  ...
  User, Phone, Calendar, FileText, Receipt, Activity, ChevronRight,
...
function PaymentModal({
  appointment, patientId, onClose, onSuccess
}: {
  appointment: Appointment; patientId: string; onClose: () => void; onSuccess: () => void;
}) {
...
function BillingTab({ patient, appointments, onRefresh }: { patient: Patient; appointments: Appointment[]; onRefresh: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Print button */}
      {appointments.length > 0 && (
        <button onClick={handlePrint} className="no-print flex items-center gap-2 px-4 py-2 border border-slate-300 dark:border-slate-700 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition">
      ...
      {/* Payment modals */}
      {payingAppointment && (
        <PaymentModal appointment={payingAppointment} patientId={patient.id}
      ...
      {activeTab === "medical" && (
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-8 text-center">
            ...
            <h3 className="font-semibold text-slate-700 dark:text-slate-300 mb-1">Medical Records</h3>
            <p className="text-sm text-slate-500">Prescriptions and medical reports will appear here.</p>
            <p className="text-xs text-slate-400 mt-2">Records are created by the doctor during a consultation visit.</p>
          </div>
        </div>
      )}
    </div>
